...
describe('createStakingPool', function () {
  ...
  it('should create and initialize a new pool minimal beacon proxy pool', async function () {
    ...
    expect(beacon).to.be.equal(cover.target);
  ...
  it('should fail to create a new pool called from pooled staking - Not a member', async function () {
    ...
    const [poolId] = await stakingProducts.connect(fixture.pooledStakingSigner).createStakingPool.staticCall(
      true, // isPrivatePool,
      initialPoolFee,
      maxPoolFee,
      [],
  ...
  it('should fail to initialize products with targetPrice below default minimum', async function () {
    ...
    const products = [{ ...productInitializationParams[0], targetPrice: DEFAULT_MIN_PRICE_RATIO - 1n }];
    await expect(
      stakingProducts.connect(stakingPoolCreator).createStakingPool(
        false, // isPrivatePool,
        initialPoolFee,
