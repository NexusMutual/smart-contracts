...
contract governance2 is Iupgradable {
    ...
    /// @dev changes the status of the proposals.
    /// @param id proposal.
    function changeProposalStatus(uint id) {
        require(gd.getProposalOwner(id) == msg.sender && gd.getProposalStatus(id) == 0);
    ...
    function editProposal(uint id, string sd, string ld) {
        require(msg.sender == gd.getProposalOwner(id) && gd.getProposalStatus(id) == 0);
    ...
    function addProposal(string shortDesc, string longDesc, address[] _effect, uint[] value, uint16 cat, bytes16[] options) {
        ...
        require(cat != 2 || gd.checkBurnVoterTokenAgaintClaim(value[0], _effect[0]) != 1);
    ...
    function categorizeProposal(uint id, uint16 cat, address[] _effect, uint[] val, bytes16[] options) {
        ...
        require(gd.isAB(msg.sender) != false);
        require(gd.isProposalCategorised(id) != 1);
