...
class App {
  constructor(config) {
    ...
    this.forceParse = config.forceParse || []; // Which files should be force parsed during instrumentation
  ...
  generateCoverageEnvironment() {
    ...
    try {
      ...
      // Identify folders to force parse
      this.forceParse.forEach(item => {
        if (path.extname(item) !== '.sol') this.forceParseFolders.push(item);
      });
  ...
  instrumentTarget() {
    ...
    this.forceParse = this.forceParse.map(
      contract => `${this.coverageDir}/contracts/${contract}`
    );
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        if (!this.skipFiles.includes(file) && !this.inSkippedFolder(file)) {
          ...
          const canonicalPath = file.split('/coverageEnv').join(working);
    ...
    shell
      ...
      .forEach(file => {
        ...
        if (
          ...
          !(this.forceParse.includes(file) || this.inForceParseFolder(file))
        ) {
          this.log('Skipping post-processing of ', file);
          return;
        }
        this.log('Post-processing ', file);
        const contractPath = this.platformNeutralPath(file);
        const contract = fs.readFileSync(contractPath).toString();
    ...
    return new Promise((resolve, reject) => {
      if (!this.norpc) {
        ...
        const command = require.resolve(
          'ethereumjs-testrpc-sc/build/cli.node.js'
        );
  ...
  /**
   * Determines if a file is in a folder marked force parse.
   * @param  {String} file file path
   * @return {Boolean}
   */
  inForceParseFolder(file) {
    ...
    this.forceParseFolders.forEach(folderToParse => {
      ...
      if (file.indexOf(folderToParse) === 0) shouldParse = true;
    });
    return shouldParse;
  }
