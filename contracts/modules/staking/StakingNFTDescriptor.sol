...
contract StakingNFTDescriptor is IStakingNFTDescriptor, FloatingPoint {
  ...
  returns (string memory depositInfo, uint totalStake, uint pendingRewards) {
    ...
    for (uint i = 0; i < MAX_ACTIVE_TRANCHES; i++) {
      ...
      depositInfo = appendWithNewline(
        depositInfo,
        " -",
        append(toFloat(stake, NXM_DECIMALS), " NXM will expire at tranche: ", uint((block.timestamp / TRANCHE_DURATION) + i).toString()),
        append(" (", dateString, ")")
      );
    ...
    return abi.encodePacked(
      string(
        abi.encodePacked(
          ...
          '<text class="cls-8" x="93%" y="40%" text-anchor="end"><tspan>', toFloat(stakeDescription.stakeAmount, NXM_DECIMALS),' NXM</tspan></text>',
