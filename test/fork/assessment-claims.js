...
const { getFundedSigner, getSigner, setUSDCBalance } = require('./utils');
...
const createCover = async (
  ...
  const maxPremiumInAsset = amount / 1000n; // 0.1%
...
describe('claim assessment', function () {
  ...
  it('should run setup - add assessors and configure assessment', async function () {
    ...
    const [claimant, assessor1, assessor2, assessor3, assessor4] = members;
    ...
    const tx = await this.assessment.addAssessorsToGroup(assessorMemberIds, 0, { gasLimit: 21e6 });
    await tx.wait();
    ...
    console.log(`Setup complete: ${assessorMemberIds.length} assessors added to group ${groupId}`);
  });
  ...
  it('Phase 2: USDC claim with fraud detection and governance intervention', async function () {
    ...
    const newAssessor = await getSigner('0x2a156b05ae6ab6ea14b4b113bf679e82973bdb58');
