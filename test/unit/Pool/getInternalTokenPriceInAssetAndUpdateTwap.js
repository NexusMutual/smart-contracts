...
describe('getInternalTokenPriceInAssetAndUpdateTwap', function () {
  it('reverts if invalid asset id is passed', async function () {
    ...
    await expect(pool.getInternalTokenPriceInAssetAndUpdateTwap(3)).to.be.revertedWithCustomError(
      pool,
      'InvalidAssetId',
    );
  });
  ...
  it('returns correct internal price in asset', async function () {
    ...
    const assetAmount = await pool.getInternalTokenPriceInAssetAndUpdateTwap.staticCall(1);
