...
function bnRoughlyEqual (actual, expected, message, maxDelta = new BN(1)) {

  const actualBN = toBN(actual);
  const expectedBN = toBN(expected);
  const error = message || `expected ${actualBN.toString()} to equal ${expectedBN.toString()}`;

  if ((actualBN.lte(expectedBN) && expectedBN.sub(actualBN).lte(maxDelta))
    || (actualBN.gt(expectedBN) && actualBN.sub(expectedBN).lte(maxDelta))) {
    return;
  }
