...
describe('unstakeAllFor', function () {
  it("decreases the staker's stake to 0", async function () {
    ...
    await assessment.connect(otherUser).unstakeAllFor(user.address);
  ...
  it("reverts if less than stakeLockupPeriodInDays passed since the staker's last vote", async function () {
    ...
    const unstakeForBeforeExpiry = assessment.connect(otherUser).unstakeAllFor(user.address);
    ...
    for (let dayCount = 1; dayCount < stakeLockupPeriodInDays; dayCount++) {
      ...
      const unstakeFor = assessment.connect(otherUser).unstakeAllFor(user.address);
    ...
    const unstakeForAtExpiry = assessment.connect(otherUser).unstakeAllFor(user.address);
  ...
  it('does NOT revert if user has no stake to unstake', async function () {
    ...
    await expect(assessment.connect(otherUser).unstakeAllFor(user.address)).to.not.be.reverted;
  });
  ...
  it('emits StakeWithdrawn event with staker, destination and amount', async function () {
    ...
    await expect(assessment.connect(otherUser).unstakeAllFor(user.address))
