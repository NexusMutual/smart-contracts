...
const r_dai = "0xdcaa177410672d90890f1c0a42a965b3af9026c04caedbce9731cb43827e8556";
const s_dai = "0x2b9f34e81cbb79f9af4b8908a7ef8fdb5875dedf5a69f84cd6a80d2a4cc8efff";
...
describe('Contract: Quotation', function () {
    ...
    it('should return correct cover details purchased with ether', async function () {
        ...
        if (cdetails2[1] == "0x45544800" && cdetails1[1] == PNAME && cdetails1[2] == coverHolder && cdetails1[3] == smartConAdd) {
            checkd = true;
        }
    ...
    it('should able to purchase cover with NXM', async function () {
        ...
        let initialLockedCN = await nxmtk2.totalBalanceCNOfUser(coverHolder);
        let initialAvailableToken = await cr.getTotalStakeCommission(member); // member=staker for smart contract
        await qt.makeCoverUsingNXMTokens(PID, coverDetails, coverPeriod, "ETH", smartConAdd, v, r, s, {from: coverHolder});
        let presentLockedCN = await nxmtk2.totalBalanceCNOfUser(coverHolder);
        let presentAvailableToken = await cr.getTotalStakeCommission(member); // staker should get 20% of premium.
        let newLockedCN = (initialLockedCN.plus(BN_5.times(new BigNumber(coverDetails[2].toString()).div(BN_100)))).toFixed(0);
        let newAvailableToken = (initialAvailableToken.plus(BN_20.times(new BigNumber(coverDetails[2].toString()).div(BN_100)))).toFixed(0);
    ...
    it('should able to purchase cover using currency assest i.e. DAI ', async function () {
        ...
        await P1.makeCoverUsingCA(PID, smartConAdd,"DAI", coverDetailsDai, coverPeriod, v, r_dai, s_dai, {from: coverHolder});
        let presentLockedCN = await nxmtk2.totalBalanceCNOfUser(coverHolder);
        let presentCAbalance = await cad.balanceOf(coverHolder);
        let presentAvailableToken = await cr.getTotalStakeCommission(member); // staker should get 20% of premium.
        let presentPoolBalanceOfCA = await cad.balanceOf(P1.address);
        let newLockedCN = (initialLockedCN.plus(BN_5.times(new BigNumber(coverDetailsDai[2].toString()).div(BN_100)))).toFixed(0);
        let newAvailableToken = (initialAvailableToken.plus(BN_20.times(new BigNumber(coverDetailsDai[2].toString()).div(BN_100)))).toFixed(0);
        presentCAbalance.should.be.bignumber.equal(initialCAbalance.minus(new BigNumber(coverDetailsDai[1].toString())));
        presentPoolBalanceOfCA.should.be.bignumber.equal(initialPoolBalanceOfCA.plus(new BigNumber(coverDetailsDai[1].toString())));
        newLockedCN.should.be.bignumber.equal(presentLockedCN);
        newAvailableToken.should.be.bignumber.equal(presentAvailableToken);
    });
    ...
    it('should return correct cover details purchased with DAI', async function () {
        ...
        if (cdetails2[1] == "0x44414900" && cdetails1[1] == PNAME && cdetails1[2] == coverHolder && cdetails1[3] == smartConAdd) {
            checkd = true;
        }
        checkd.should.equal(true);
    });
