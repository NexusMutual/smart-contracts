...
describe('createStakingPool', function () {
  ...
  beforeEach( async function () {
    const { coverProducts, initialProducts } = this;
    ...
    await coverProducts.setProduct({ ...coverProductTemplate, initialPriceRatio: coverProductTemplate.initialPriceRatio }, productId);
  ...
  it('reverts if system is paused', async function () {
    const { stakingProducts, master } = this;
  ...
  it('should create and initialize a new pool minimal beacon proxy pool', async function () {
    const { cover, stakingPoolFactory, stakingProducts, coverProducts } = this;
  ...
  it('should fail to create a new pool called from pooled staking - Not a member', async function () {
    ...
    await expect(
      stakingProducts.connect(this.pooledStakingSigner).createStakingPool(
        true, // isPrivatePool,
        initialPoolFee,
        maxPoolFee,
        initialProducts,
  ...
  it('emits StakingPoolCreated event', async function () {
    const { stakingProducts, stakingPoolFactory } = this;
    const [stakingPoolCreator] = this.accounts.members;
    const { initialPoolFee, maxPoolFee, productInitializationParams, ipfsDescriptionHash } = newPoolFixture;
