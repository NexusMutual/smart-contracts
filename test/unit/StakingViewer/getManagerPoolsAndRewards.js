...
describe('getManagerPoolsAndRewards', function () {
  ...
  it('returns empty arrays and zero total rewards for address with no pools', async function () {
    const fixture = await loadFixture(setup);
    const [nonPoolManager] = fixture.accounts.nonMembers;
    const { stakingViewer } = fixture.contracts;

    const { pools, rewards, totalRewards } = await stakingViewer.getManagerPoolsAndRewards(nonPoolManager.address);
