...
contract SwapOperator is ISwapOperator, RegistryAware {
  ...
  function placeOrder(GPv2Order.Data calldata order, bytes calldata orderUID) public onlyController {
    ...
    {
      address transferedAsset = swapOperationType == SwapOperationType.EthToAsset ? ETH : order.sellToken;
  ...
  function swapEnzymeVaultShareForETH(uint amountIn, uint amountOutMin) external onlyController {
  ...
  function setSafeTransferAssetAllowed(address asset, bool allowed) external onlyContracts(C_GOVERNOR) {
    allowedSafeTransferAssets[asset] = allowed;
  }

  /// @dev Create a request for transfering assets to the safe
  function requestAssetTransfer(address asset, uint amount) external onlySafe {
  ...
  function requestAssetSwap(
    ...
    swapRequest = SwapRequest(assetIn, assetOut, amountIn, amountOutMin);
  }
