...
describe('burnStake', function () {
  ...
  it('does not deallocate after cover expiry', async function () {
    ...
    const initiallyAllocatedTotal = initialAllocations.reduce((acc, val) => acc + val, 0n);
    ...
    const burnParams = {
      ...
      period,
      deallocationAmount: initiallyAllocatedTotal / 2n, // claimed half of the cover amount
    };
    ...
    const finallyAllocatedTotal = finalAllocations.reduce((acc, val) => acc + val, 0n);
