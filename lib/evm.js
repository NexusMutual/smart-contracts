...
// Lazy load ethers to avoid circular dependency
let ethers;
const getEthers = () => ethers || (ethers = require('hardhat').ethers);
...
const hex = n => getEthers().toQuantity(n);
...
const methods = {
  ...
  hardhat: send => ({
    ...
    setStorageAt: async (address, slot, value) => send('hardhat_setStorageAt', [address, slot, value]),
    setNextBlockBaseFee: async fee => send('hardhat_setNextBlockBaseFeePerGas', [hex(fee)]),
  }),

  tenderly: send => ({
    // on tenderly all accounts can send transactions without impersonation
    impersonate: () => {},
    ...
    setStorageAt: async (address, slot, value) => send('tenderly_setStorageAt', [address, slot, value]),
