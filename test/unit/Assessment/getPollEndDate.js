...
const expectClaimPollEndDate = assessment => async (id, expected) => {
  const pollEnd = await assessment.getPollEndDate(EVENT_TYPE.CLAIM, id);
  assert(
    pollEnd === expected,
...
describe('getPollEndDate', function () {
  it('should end after MIN_VOTING_PERIOD_DAYS if if no votes are cast', async function () {
    const { assessment } = this.contracts;
    const expectPollEndDateOfClaim = expectClaimPollEndDate(assessment);
