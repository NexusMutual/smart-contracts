...
const tenderlyFactory = web3 => async txPromise => {
  ...
  const { wrappedProvider } = web3.currentProvider;
  assert(wrappedProvider.host, 'Tenderly: network provider required to export tx');

  const providerURL = wrappedProvider.host.replace(/^http:\/\//, '');
