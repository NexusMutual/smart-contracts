...
contract governanceData is Iupgradable {
    ...
    /// @dev Gets pending proposal start variable, 
    //       which is the lowest proposal id with status number < 2(either draft status or pending vote status).
    ...
    function getProposalById1(uint id, address _add) 
    constant 
    returns(
        int8 userABVote, 
        uint proposalId, 
        ...
        return (
            userProposalABVote[_add][id], 
            allPro[id].id, allPro[id].owner, 
            allPro[id].shortDesc, allPro[id].longDesc, 
            userProposalMemberVote[_add][id], 
            allPro[id].date_upd
            );
    }
    ...
    /// @dev Maps the given vote Id against the given Advisory board member's address.
    //        Maintains the record of all the votes an AB member has given till date.
    ...
    /// @dev Adds the given voter Id against the given Proposal's Id.
    //               Maintains the record of all the votes that have been given by the Advisory board members to a Proposal.
    ...
    function getproposalIdVersions(uint proposalid, uint ind) constant returns(uint64 version, string shortDesc, string longDesc, uint date_add) {
        return (
            proposalIdVersions[proposalid][ind].version, 
            proposalIdVersions[proposalid][ind].shortDesc, 
            proposalIdVersions[proposalid][ind].longDesc, 
            proposalIdVersions[proposalid][ind].date_add
            );
    }
