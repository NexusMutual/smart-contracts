...
describe('cbBTC', function () {
  ...
  it.skip('Deploy new PriceFeedOracle contract', async function () {
    const poolAssets = {
      dai: {
    ...
    console.log('this.priceFeedOracle.address: ', this.priceFeedOracle.address);
  });

  it.skip('Update the PriceFeedOracle in the Pool contract', async function () {
    const poolPriceFeedOracleBefore = await this.pool.priceFeedOracle();
    console.log('poolPriceFeedOracleBefore: ', poolPriceFeedOracleBefore);
    console.log('this.priceFeedOracle.address: ', this.priceFeedOracle.address);
  ...
  it.skip('Add cbBTC as an asset to the pool', async function () {
    ...
    console.log('poolValueInEthBefore: ', poolValueInEthBefore.toString());
    ...
    console.log('poolValueInEthAfter: ', poolValueInEthAfter.toString());
    expect(poolValueInEthAfter).to.be.gt(poolValueInEthBefore);

    const poolAssetsAfter = await this.pool.getAssets();
    console.debug('poolAssets: ', require('util').inspect(poolAssetsAfter, { depth: null }));
  ...
  it.skip('fail to buy cover that only supports DAI', async function () {
    ...
    await expect(buyCoverOnlyDai).to.be.revertedWithCustomError(this.cover, 'CoverAssetNotSupported');
  });

  it.skip('fail to buy cover that only supports ETH', async function () {
    const coverBuyerAddress = await this.coverBuyer.getAddress();
    ...
    await expect(buyCoverOnlyEth).to.be.revertedWithCustomError(this.cover, 'CoverAssetNotSupported');
  });

  it.skip('Buy cover with cbBTC', async function () {
    const coverBuyerAddress = await this.coverBuyer.getAddress();
    ...
    console.log('poolCbBtcBalanceBefore: ', poolCbBtcBalanceBefore.toString());
    ...
    console.log('poolCbBtcBalanceAfter: ', poolCbBtcBalanceAfter.toString());
  ...
  it('Places a swap order', async function () {
    ...
    const buyAmount = await this.priceFeedOracle.getAssetForEth(
      Address.CBBTC_ADDRESS,
      sellAmount, // .sub(parseEther('0.1')),
    );
    console.log('---------------REAL ORDER-------------------');
    console.log('sellAmount: ', formatEther(sellAmount), ' ETH'); // 10 ETH
    console.log('buyAmount: ', formatUnits(buyAmount, 8), ' CBBTC'); // 0.39 CBBTC
    console.log('----------------------------------');
    ...
    const otherSideOrder = {
      ...
      kind: 'sell',
      partiallyFillable: false,
      sellToken: Address.CBBTC_ADDRESS.toLowerCase(),
      buyToken: Address.WETH_ADDRESS.toLocaleLowerCase(),
      sellAmount: buyAmount, // .add(parseUnits('0.01', 8)),
    ...
    console.log('----------------------------------');
    console.log('cbBtcEthPrice: ', formatEther(cbBtcEthPrice)); // 25.2 ETH
    console.log('ethCbBtcPrice: ', formatUnits(ethCbBtcPrice, 8)); // 0.03968129 BTC
    console.log('----------------------------------');
    console.log('buyAmount: ', formatUnits(buyAmount, 8));
    console.log('imaginaryBuyAmount: ', formatUnits(imaginaryBuyAmount, 8));
    console.log('----------------------------------');

    // NOTE: prices address keys needs to be check summed
    const prices = {
      [Address.CBBTC_ADDRESS]: cbBtcEthPrice, // 25.2 ETH
      [Address.WETH_ADDRESS]: ethCbBtcPrice.mul(10 ** ),
    };

    console.log('---------------REAL ORDER PRICING-------------------');
    // 0.39 CBTC * 25.2 ETH ~ ETH
    console.log('BUY AMOUNT * BUY PRICE:  ', order.buyAmount.mul(prices[Address.CBBTC_ADDRESS]).toString());
    // 
    // 0.39 CBTC * 25.2 ETH = 10 ETH * ??
    // (0.39 CBTC * 25.2 ETH) / 10 ETH = ??
    ...
    console.log('cowswapSettlement.settle done');
    await this.swapOperator.connect(this.swapController).closeOrder(contractOrder);
    console.log('swapOperator.closeOrder');
    ...
    console.log('cbBTCBalanceAfter: ', cbBTCBalanceAfter.toString());

    // 1 BTC to ETH
    const cbBtcEthPrice2 = await this.priceFeedOracle.getEthForAsset(Address.CBBTC_ADDRESS, parseUnits('1', 8));
    // 1 ETH to BTC
    const ethCbBtcPrice2 = await this.priceFeedOracle.getAssetForEth(Address.CBBTC_ADDRESS, parseEther('1'));
    console.log('ethCbBTCPrice RATE: ', formatUnits(ethCbBtcPrice2, 8).toString());
    console.log('cbBTCEthPrice RATE: ', formatEther(cbBtcEthPrice2).toString());
    ...
    console.log('cbBTCBalanceIncrease: ', cbBTCBalanceIncrease.toString());
    console.log('ethBalanceDecrease: ', ethBalanceDecrease.toString());
