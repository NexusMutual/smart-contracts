...
describe('claimable', function () {
  it('returns 0 when no rewards have been distributed', async function () {
    ...
    expect(await rewards.claimable(token.target, staker1.address, data)).to.equal(0n);
  });
  ...
  it('returns 0 when staker has no shares', async function () {
    ...
    expect(await rewards.claimable(token.target, staker1.address, claimableData)).to.equal(0n);
  });
  ...
  it('returns correct amount for single reward distribution', async function () {
    ...
    expect(await rewards.claimable(token.target, staker1.address, claimableData)).to.equal(expectedReward);
  });
  ...
  it('returns correct amounts for various scenarios without modifying state', async function () {
    ...
    expect(claimable3).to.equal(expected3);
  ...
  it('handles proportional shares correctly with active + withdrawal shares', async function () {
    ...
    expect(await rewards.claimable(token.target, staker1.address, claimableData)).to.equal(expected1);
    expect(await rewards.claimable(token.target, staker2.address, claimableData)).to.equal(expected2);
  });
  ...
  it('handles rewards after actual claim correctly', async function () {
    ...
    // should be 0 claimable after claiming
    expect(await rewards.claimable(token.target, staker1.address, claimableData)).to.equal(0n);
  });
  ...
  it('stops at maxRewards when fewer than total rewards (processed < maxRewards exits first)', async function () {
    ...
    expect(await rewards.rewardsLength(token.target, NETWORK_ID)).to.equal(5n);
  ...
  it('stops at rewardsLength when maxRewards exceeds available (rewardIndex < length)', async function () {
    ...
    expect(await rewards.rewardsLength(token.target, NETWORK_ID)).to.equal(3n);

    // request maxRewards = 10 (more than available)
    ...
    // request maxRewards = 100 (much more than available)
    ...
    // request maxRewards = 3 (exactly the available)
    const maxRewards3 = abi.encode(['address', 'uint256'], [NETWORK_ID, 3n]);
    const claimable3 = await rewards.claimable(token.target, staker1.address, maxRewards3);
    ...
    expect(claimable10).to.equal(claimable3);
