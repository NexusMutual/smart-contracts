...
describe('setSwapAssetAmount', function () {
  it('is only callabe by swap operator', async function () {
    ...
    await expect(pool.setSwapAssetAmount(ETH_ADDRESS, parseEther('0.123'))).to.be.revertedWith(
    ...
    // Call should succeed
    await pool.connect(swapOperator).setSwapAssetAmount(ETH_ADDRESS, parseEther('0.123'));
  });
  ...
  it('sets the swapValue value', async function () {
    ...
    expect(await pool.assetInSwapOperator()).to.eq(parseEther('0.123'));
  });
});
