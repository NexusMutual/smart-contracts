...
describe('transferFunds', function () {
  it('should transfer funds (ETH) to owner', async function () {
    ...
    const tx = await coverBroker.connect(coverBrokerOwner).transferFunds(ETH);
  ...
  it('should transfer funds (nonETH) to owner', async function () {
    const fixture = await loadFixture(setup);
    const {
    ...
    await coverBroker.connect(coverBrokerOwner).transferFunds(dai.address);
  ...
  it('should fail to transfer funds if the caller is not the owner', async function () {
    const fixture = await loadFixture(setup);
    ...
    await expect(coverBroker.connect(nonOwner).transferFunds(ETH)).to.revertedWith('Ownable: caller is not the owner');
