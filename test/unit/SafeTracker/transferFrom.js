...
describe('transferFrom', function () {
  it("should revert if amount is not 0 and the caller aren't pool and swapOperator", async function () {
    ...
    await expect(
      safeTracker.connect(defaultSender).transferFrom(memberSender.address, memberRecipient.address, 100),
    ).to.be.revertedWithCustomError(safeTracker, 'AmountExceedsBalance');
  });
