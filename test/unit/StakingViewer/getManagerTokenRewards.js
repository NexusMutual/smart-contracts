...
describe('getManagerTokenRewardsByAddr', function () {
  ...
  it('returns empty rewards for an address with no managed pools', async function () {
    const fixture = await loadFixture(setup);
    const [nonPoolManager] = fixture.accounts.nonMembers;
    const { stakingViewer } = fixture.contracts;

    const rewards = await stakingViewer.getManagerTokenRewardsByAddr(nonPoolManager.address);
