...
export function getObservationIndex(timestamp, { PERIOD_SIZE, GRANULARITY }) {
 ...
 */
export function calculateInternalPrice(currentState, observations, capital, supply, currentTimestamp, constants) {
 ...
 */
export async function getInternalPrice(ramm, pool, tokenController, timestamp) {
...
export function calculatePremium(amount, rate, period, price, allocationUnit) {
...
export function calculateFirstTrancheId(lastBlock, period, gracePeriod) {
  return Math.floor((lastBlock.timestamp + Number(period) + Number(gracePeriod)) / (91 * 24 * 3600));
}

// TODO: eject from lib
export async function stakeOnly({ stakingPool, staker, period, gracePeriod, trancheIdOffset }) {
  // Staking inputs
...
// TODO: eject from lib
export async function stake({ contracts, stakingPool, staker, productId, period, gracePeriod, amount = 0 }) {
