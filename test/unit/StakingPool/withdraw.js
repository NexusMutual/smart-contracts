...
describe('withdraw', function () {
  ...
  it('allows to withdraw only stake', async function () {
    ...
    const expectedShares = sqrt(depositAmount);
  ...
  it('allows to withdraw only rewards', async function () {
    ...
    const expectedShares = sqrt(amount);
  ...
  it('allows to withdraw stake and rewards from multiple tranches', async function () {
    ...
    let rewardsWithdrawn = 0n;
    let stakeWithdrawn = 0n;
  ...
  it('allows withdrawing rewards multiple times', async function () {
    ...
    await stakingPool
      ...
      .multicall(
        [
          await stakingPool.depositTo.populateTransaction(amount, lastTrancheId, 0, alice.address),
          await stakingPool.depositTo.populateTransaction(amount, lastTrancheId, 0, bob.address),
        ].map(tx => tx.data),
      );
