...
describe('Contract: NXMToken1', function() {
  ...
  it('should able to lock tokens under Claim Assesment', async function() {
    ...
    availableTokens.should.be.bignumber.equal(
      initialAvailableTokens.minus(Tokens)
    );
  ...
  it('should able to extend amount of tokens for Claim Assesment', async function() {
    ...
    newTokenAvailable.should.be.bignumber.equal(
      initialTokenAvailable.minus(ExtendLockAmount)
    );
  });
  ...
  it('should able to transfer tokens to any other member', async function() {
    ...
    presentTokenOfMember.should.be.bignumber.equal(
      initialTokenOfMember.minus(tokensToTransfer)
    );
    presentTokenOfReceiver.should.be.bignumber.equal(
      initialTokenOfReceiver.plus(tokensToTransfer)
    );
  });

  it('should able to stake NXMs on Smart Contracts', async function() {
    let initialTokenAvailable = await nxmtk1.balanceOf(member);
    let initialStaked = await nxmtd.getTotalStakedAmtByStakerAgainstScAddress(
      member,
      contractAdd
    );
    await nxmtk2.addStake(contractAdd, stakeTokens, { from: member });
    let currentTokenAvailable = await nxmtk1.balanceOf(member);
    let currentStaked = await nxmtd.getTotalStakedAmtByStakerAgainstScAddress(
      member,
      contractAdd
    );
    currentTokenAvailable.should.be.bignumber.equal(
      initialTokenAvailable.minus(stakeTokens)
    );
