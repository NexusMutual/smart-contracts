...
describe.only('accumulateReward', function () {
  ...
  it('should not push the same contract multiple times in the same round', async function () {
    ...
    const start = await staking.REWARD_ROUND_DURATION();
    const now = await time.latest();

    console.log(`currentRound: ${currentRound}, current time: ${now}, rounds start: ${start}`);
