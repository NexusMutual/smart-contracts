...
contract NXMaster is Governed {
  ...
  onlyAuthorizedToGovern {
    ...
    // Using extra varible to get rid of if condition.
    if (_type == 1) {
  ...
  function addEmergencyPause(bool _pause, bytes4 _by) public {
    ...
    if (_pause == false) {
      ...
      // Process claims submitted while EP was on
      c1.startAllPendingClaimsVoting();
      // Resume voting on all pending claims
    }
  }
  ...
  function startEmergencyPause() public onlyAuthorizedToGovern {
    addEmergencyPause(true, "AB");
    //Start Emergency Pause
    Pool1 p1 = Pool1(allContractVersions["P1"]);
    p1.closeEmergencyPause(pauseTime);
    //oraclize callback of 4 weeks
    Claims c1 = Claims(allContractVersions["CL"]);
    c1.pauseAllPendingClaimsVoting();
