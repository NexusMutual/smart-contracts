...
contract NXMasterNew is Governed {
  ...
  onlyAuthorizedToGovern
  {
    ...
    for (uint i=0; i<_contractsName.length; i++) {
      ...
      up = Iupgradable(allContractVersions[_contractsName[i]]);
      up.changeMasterAddress(address(this));
    }
  ...
  function canCall(uint _claimId) internal returns(bool)
  {
    cd = ClaimsData(getLatestAddress("CD"));
