...
start_ganache() {
  if [ "$SOLIDITY_COVERAGE" = true ]; then
    ...
    node_modules/.bin/ganache-cli --gasLimit 10000000 -p "$ganache_port" -i 5777 -m "grocery obvious wire insane limit weather parade parrot patrol stock blast ivory" --hardfork constantinople -a 30 -e 10000000 > /dev/null &
  fi
